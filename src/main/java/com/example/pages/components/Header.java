    ...
    public void openPlansAndPricing() {
        ...
        try {
            ((JavascriptExecutor) driver)
                    .executeScript("arguments[0].scrollIntoView({block:'center', inline:'center'});", link);
            Waiter.clickable(link);
            new Actions(driver).moveToElement(link).pause(Duration.ofMillis(80)).click().perform();
